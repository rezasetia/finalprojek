...
<body>
    <article id="about" class="my-32">
        <section class="relative w-full h-screen bg-blend-multiply flex items-center justify-center">
            <div id="default-carousel" class="relative w-full h-full" data-carousel="slide">
                <!-- Carousel wrapper -->
                <div class="relative h-full overflow-hidden rounded-lg mx-16">
                    <?php if (!empty($banners)): ?>
                        <?php foreach ($banners as $index => $banner): ?>
                            <div class="<?php echo $index === 0 ? 'block' : 'hidden'; ?> duration-700 ease-in-out" data-carousel-item>
                                <img src="<?php echo $banner['image_path']; ?>" class="absolute block w-full h-full object-cover top-0 left-0" alt="<?php echo $banner['alt_text']; ?>" />
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
