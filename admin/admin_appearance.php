...
if(isset($_POST['tambah_banner'])) {
    ...
    if ($result) {
        ...
        header("Location: admin_appearance.php");
...
// Ganti gambar banner
if(isset($_POST['ganti_banner'])) {
    $id = $_POST['id'];
    $altText = $_POST['alt_text'];
    $isActive = $_POST['is_active'];

    // Perbarui detail gambar di database
    $sql = "UPDATE banners SET alt_text = '$altText', is_active = '$isActive' WHERE id = '$id'";
    $result = $connection->query($sql);

    if($result) {
        echo "Detail gambar banner berhasil diperbarui.";
    } else {
        echo "Terjadi kesalahan saat memperbarui detail gambar banner.";
    }
}
?>
<!DOCTYPE html>
                ...
                <div class="mb-4 rounded bg-gray-50 ">
                    ...
                    <form class="flex items-center my-7 md:gap-x-14">
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" id="simple-search" placeholder="Search users" required="" class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 py-3" />
                        </div>
                            ...
                            <tbody>
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        echo "<td class='px-6 py-4 text-gray-500'>{$row['is_active']}</td>";
                                        echo "<td class='flex items-center justify-center px-6 py-4'>";
                                        echo "<a href='?edit_banner={$row['id']}' class='flex items-center justify-center gap-x-2 font-medium bg-green-600 rounded-lg text-white hover:bg-green-700 px-10 py-2.5 hover:underline'>";
